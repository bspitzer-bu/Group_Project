...
parser.add_option('--dismatout', action='store_true', help=('Output distance matrix. This is only calculated if the mBedlike'
...
def restRequest(url):
    ...
    try:
        ...
        reqH = urlopen(req, context=ssl._create_unverified_context())
        resp = reqH.read()
...
def serviceRun(email, title, params):
    ...
    try:
        ...
        reqH = urlopen(req, requestData.encode(encoding=u'utf_8', errors=u'strict'), context=ssl._create_unverified_context())
